...
class Spacenet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(os.path.join(self.img_root, self.files[index] + '_RGB.tif')).convert('LA')
        #target = cv2.imread(os.path.join(self.img_root, self.files[index] + '_GT.tif'))
        target = Image.open(os.path.join(self.img_root, self.files[index] + '_RGB.tif')).convert('RGB')
...
if __name__ == "__main__":
    ...
    spacenet_train = Spacenet(split='train')
