...
class Config:
    ...
    channels = [['red', 'green', 'blue'], ['H', 'S', 'V']]

    if len(channels) == 2:
        building_color_dist = {'RGB': building_rgb_hist, 'HSV': building_hsv_hist}
        nonbuilding_color_dist = {'RGB': nonbuilding_rgb_hist, 'HSV': nonbuilding_hsv_hist}
    else:
        if 'red' in channels:
