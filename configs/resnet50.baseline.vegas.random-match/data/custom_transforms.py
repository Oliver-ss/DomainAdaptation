...
class HistogramMatching(object):
    # image to image histogram matching
    ...
    def get_table(self, img, reference):
        mask_img = self.get_mask(img)
        mask_ref = self.get_mask(reference)
        img = img[mask_img]
        reference = reference[mask_ref]
        table = []
        for i in range(3):
            hist_img = np.histogram(img[:,i],256,[0,256])[0]
            hist_ref = np.histogram(reference[:,i],256,[0,256])[0]
            cdf_img = hist_img.cumsum() / hist_img.sum()
            cdf_ref = hist_ref.cumsum() / hist_ref.sum()
            table.append(np.interp(cdf_img, cdf_ref, np.arange(256)))
        return table

    def match_histogram(self, img, reference):
        ans = np.zeros_like(img)
        table = self.get_table(img, reference)
        for i in range(3):
            ans[:,:,i] = np.reshape(table[i][img[:,:,i].ravel()], img.shape[:2])
        return ans
...
class Remap(object):
    def __init__(self, table, ntable, channels):
        self.table = table
        self.ntable = ntable
        self.channels = channels

    def blur(self, mask, kernel=(5,5)):
        return cv2.GaussianBlur(mask, kernel, 0)
    ...
    def __call__(self, sample):
        if random.random() < 0.5:
            ...
            building = np.zeros_like(img)
            for i, color in enumerate(self.channels):
                building[:,:,i] = (np.reshape(self.table[color][img[:,:,i].ravel()], img.shape[:2]))
            nonbuilding = np.zeros_like(img)
            for i, color in enumerate(self.channels):
                nonbuilding[:,:,i] = (np.reshape(self.ntable[color][img[:,:,i].ravel()], img.shape[:2]))
            im = building*mask + nonbuilding*(np.ones_like(mask)-mask)
