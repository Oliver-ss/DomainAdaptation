...
class Test:
    def __init__(self, model_path, config, bn, save_path, save_batch, cuda=False):
        ...
        # load source domain
        self.source_set = spacenet.Spacenet(city=config.dataset, split='test', img_root=config.img_root, source_dist=dist[config.dataset])
        ...
        for city in self.target:
            test = spacenet.Spacenet(city=city, split='test', img_root=config.img_root, source_dist=dist[city])
            self.target_set.append(test)
            self.target_loader.append(DataLoader(test, batch_size=16, shuffle=False, num_workers=2))
            train = spacenet.Spacenet(city=city, split='train', img_root=config.img_root, source_dist=dist[city])
