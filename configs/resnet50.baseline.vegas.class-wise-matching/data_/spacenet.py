...
class Spacenet(data.Dataset):
    ...
    def __init__(self, city='Vegas', split='train', img_root='/usr/xtmp/satellite/spacenet/',
        ...
        self.files = []
        for f in files:
            self.files.append('/usr/xtmp/satellite/spacenet/'+f)
            self.files.append('/usr/xtmp/satellite/FakeShanghai-Vegas/'+f)
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(self.files[index] + '_RGB.tif').convert('RGB')
        #target = cv2.imread(os.path.join(self.img_root, self.files[index] + '_GT.tif'))
        target = Image.open(self.files[index] + '_GT.tif')
    ...
    def transform_tr(self, sample):
        if self.color_dist == {}:
            composed_transforms = transforms.Compose([
                ...
                #tr.ConvertFromInts(),
                #tr.BuildingRepaint(self.color_dist),
