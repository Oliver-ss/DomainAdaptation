...
def compute_mean_variance(city='Vegas'):
    ...
    train_set = spacenet.Spacenet(city=city, split='train', img_root='/usr/xtmp/satellite/spacenet/')
    ...
    fake_train_set = spacenet.Spacenet(city=city, split='train', img_root='/usr/xtmp/satellite/FakeShanghai-Vegas/')
    fake_loader = DataLoader(fake_train_set, batch_size=config.batch_size, shuffle=False,
                              num_workers=config.train_num_workers, drop_last=True)
    mean = 0.
    std = 0.
    nb_samples = 0.
...
if __name__ == '__main__':
    ...
    with open('../../scripts/fake_mean_std.json', 'w') as f:
        json.dump(output, f)
