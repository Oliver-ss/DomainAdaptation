...
def TranslateTable(table, source, target):
    source_iou = table[source]
    ious = [i[1] for i in source_iou.items()]
    bins = [np.percentile(ious, i) for i in range(0,100,10)]
    ans = {}
    for b in bins:
        ans[b] = []
    for item in table[target].items():
        b = FindNearest(item[1], bins)
        ans[b].append(item[0])
    return ans, bins
...
class Spacenet(data.Dataset):
    ...
    def __init__(self, city='Vegas', split='train', img_root='/usr/xtmp/satellite/spacenet/',
        ...
        with open('../../scripts/vegas_pseudo_labels.json') as f:
            pair_file = json.load(f)
        self.target_files, self.bins = TranslateTable(pair_file, city, target)
        if not self.files:
    ...
    def __getitem__(self, index):
        ...
        ratio = FindNearest(GetRatio(img, label), self.bins)
        if str(ratio) in self.target_files.keys() and len(self.target_files[str(ratio)]) != 0:
